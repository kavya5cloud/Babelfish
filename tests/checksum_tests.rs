...
fn validates_multiple_crc16_modbus_frames() {
    ...
    frame1.extend_from_slice(&crc1.to_le_bytes());

    let mut frame2 = data2.clone();
    frame2.extend_from_slice(&crc2.to_le_bytes());

    let invalid_frame = vec![0xAA, 0xBB, 0xCC, 0xDD, 0x00, 0x00];
...
fn validates_100_synthetic_frames() {
    ...
    for i in 0u8..100 {
        ...
        frames.push(common::make_crc16_modbus_frame(&data));
    }
...
fn identifies_crc16_modbus_from_unknown_frames() {
    ...
    let crc_candidate = candidates
        .iter()
        .find(|candidate| candidate.algorithm.name() == "CRC16/MODBUS")
    ...
    let xor_candidate = candidates
        .iter()
        .find(|candidate| candidate.algorithm.name() == "XOR")
...
fn returns_crc16_modbus_as_best_candidate() {
    ...
    assert_eq!(best.algorithm.name(), "CRC16/MODBUS");

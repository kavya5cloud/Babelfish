...
fn build_protocol_fields(
    fields: &[FieldInterpretation],
    multi_byte_fields: &[MultiByteInterpretation],
    ...
    /*
     * A unique highest-scoring multi-byte interpretation is considered
     * safe to represent as a typed field.
     *
     * If multiple candidates have the same best score, we intentionally
     * do not collapse them into one interpretation.
     */
    ...
    for (index, field) in fields.iter().enumerate() {
        ...
        /*
         * Field positions are relative to checksum.coverage_start
         * in the underlying hypothesis. The ProtocolField offset is
         * therefore normalized to the complete frame below by the
         * caller.
         */
        ...
        if let Some(multi) = unique_multi_byte {
            ...
            if offset > multi.start && offset < multi.start + multi.width {
                continue;
            }
        }
